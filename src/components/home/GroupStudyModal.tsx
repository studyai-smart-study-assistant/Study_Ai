...
export default function GroupStudyModal({ open, onOpenChange, userId, onOpenGroup }: GroupStudyModalProps) {
  ...
  const handleCreateGroup = async () => {
    ...
    if (error || !data) {
      console.error(error);
      if (error?.code === RPC_NOT_FOUND_CODE) {
        toast.error('Backend migration pending: create_study_group function not found.');
      } else {
        toast.error(error?.message || 'Unable to create group right now.');
      }
      return;
    }
    ...
    onOpenChange(false);
    onOpenGroup?.((data as StudyGroup).id);
  };

  const handleJoinGroup = async () => {
    if (!userId) {
      toast.error('Please login first to join a group.');
      return;
    }
    ...
    if (error || !data) {
      if (error?.code === RPC_NOT_FOUND_CODE) {
        toast.error('Backend migration pending: join_study_group_by_code function not found.');
        return;
      }

      const message = error?.message?.toLowerCase().includes('limit')
        ? 'This group already has 50 members.'
        : (error?.message || 'Invalid code. Please verify and try again.');
      toast.error(message);
      return;
    }
