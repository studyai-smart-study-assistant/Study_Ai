...
const EnhancedNotesGeneratorWrapper: React.FC<EnhancedNotesGeneratorWrapperProps> = ({ onSendMessage }) => {
  ...
  useEffect(() => {
    if (currentUser) {
      const access = canAccessFeature(currentUser.uid, 'notes_generation');
      setHasAccess(access);
    }
  }, [currentUser]);
  ...
  const wrappedOnSendMessage = async (message: string) => {
    if (!hasAccess) {
      toast.error('कृपया पहले Notes Generator को अनलॉक करें');
      return;
    }
    ...
    return (
      <div className="w-full p-6 text-center bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Notes Generator का उपयोग करने के लिए पहले लॉगिन करें
        </p>
      </div>
    );
  }
  ...
  if (!hasAccess) {
    ...
    return (
      <div className="w-full p-6 text-center bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
        ...
        <h3 className="text-lg font-semibold mb-2">Notes Generator अनलॉक करें</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Notes Generator का उपयोग करने के लिए 10 पॉइंट्स चाहिए।<br />
          आपके पास: {currentPoints} पॉइंट्स
        </p>
        <Button onClick={handleUnlock} disabled={isUnlocking} className="mr-2">
          {isUnlocking ? 'अनलॉक हो रहा है...' : '10 Points से अनलॉक करें'}
        </Button>
