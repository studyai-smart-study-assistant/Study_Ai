...
export async function generateResponseWithSearch(
  ...
  try {
    ...
    // If image was generated by agent, prepend it to response
    let finalResponseText = responseText;
    if (imageUrl) {
      finalResponseText = `[IMG_DATA:${imageUrl}]${responseText}`;
    }
    ...
    return { text: finalResponseText, sources, webSearchUsed, toolUsed, imageUrl };
  } catch (error: any) {
