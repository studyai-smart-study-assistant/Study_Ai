...
export async function saveCloudChats(chats: Chat[]): Promise<boolean> {
  ...
  return upsertUserPrivateData(userId, { chat_history: chats }, 'save cloud chats');
}
...
export async function saveCloudAppData(appData: Record<string, string>): Promise<boolean> {
  ...
  return upsertUserPrivateData(userId, { app_data: appData }, 'save cloud app data');
}

async function upsertUserPrivateData(
  userId: string,
  partial: Partial<Pick<UserPrivateDataRow, 'chat_history' | 'app_data'>>,
  operationLabel: string,
): Promise<boolean> {
  const { data: existing, error: readError } = await supabaseAny
    .from('user_private_data')
    .select('user_id, chat_history, app_data')
    .eq('user_id', userId)
    .maybeSingle<UserPrivateDataRow>();
  ...
  if (readError) {
    console.error(`[cloud-sync] Failed to read existing row before ${operationLabel}:`, readError);
    return false;
  }

  const payload = {
    user_id: userId,
    chat_history: partial.chat_history ?? existing?.chat_history ?? [],
    app_data: partial.app_data ?? existing?.app_data ?? {},
  };

  const { error: writeError } = await supabaseAny
    .from('user_private_data')
    .upsert(payload, { onConflict: 'user_id' });

  if (writeError) {
    console.error(`[cloud-sync] Failed to ${operationLabel}:`, writeError);
    return false;
  }
